...
import { Plus, Trash2, ArrowLeft, Save, AlertCircle, Search } from 'lucide-react'
...
function NewRulePageContent() {
  ...
  const initialAdsAccountId = searchParams.get('adsAccountId')
  ...
  const [adsAccountId, setAdsAccountId] = useState(initialAdsAccountId || '')
  ...
  const [conditions, setConditions] = useState<Condition[]>([
    { conditionGroup: 0, metric: 'cflc_cost', operator: 'gt', value: '100000' }
  ])
  ...
  const handleAddGroup = () => {
    const nextGroupId = Math.max(...conditions.map(c => c.conditionGroup)) + 1
  ...
  return (
    <div className="w-full pb-20">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-1)]">Tạo Rule mới</h1>
          <p className="text-sm text-[var(--text-3)]">Thiết lập tự động hóa tối ưu cho tài khoản Google Ads.</p>
        </div>
      </div>
      ...
      <div className="grid gap-8">
        ...
        <div className="flex items-center justify-end gap-4 mt-4">
          ...
          <Button 
            ...
            disabled={loading}
          >
            {loading ? 'Đang lưu...' : <><Save className="w-4 h-4" /> Lưu và Kích hoạt</>}
          </Button>
        </div>
      </div>
    </div>
