...
const SECTION_COLORS: Record<string, { bg: string, text: string }> = {
  'Báo cáo & Phân tích': { bg: 'bg-emerald-500/10 dark:bg-emerald-500/15', text: 'text-emerald-500 dark:text-emerald-400' },
  'Tối ưu Chiến dịch': { bg: 'bg-cyan-500/10 dark:bg-cyan-500/15', text: 'text-cyan-500 dark:text-cyan-400' },
  'AI Integration': { bg: 'bg-orange-500/10 dark:bg-orange-500/15', text: 'text-orange-500 dark:text-orange-400' },
  'Cấu hình & Tích hợp': { bg: 'bg-pink-500/10 dark:bg-pink-500/15', text: 'text-pink-500 dark:text-pink-400' },
  'Quản trị hệ thống': { bg: 'bg-violet-500/10 dark:bg-violet-500/15', text: 'text-violet-500 dark:text-violet-400' },
}
    ...
    >
      ...
      <div className="h-[64px] px-5 flex items-center justify-between border-b border-border shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          {/* Logo Mark */}
          <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center shrink-0 shadow-sm transition-transform hover:scale-105">
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-6 scrollbar-thin">
        {filteredNav.map((group) => (
          <div key={group.section} className="space-y-2">
            {/* Section label */}
            {!isSidebarCollapsed ? (
              <div className="px-3 text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-80">
                {group.section}
              </div>
            ) : (
            ...
            <div className="space-y-[4px]">
              {group.items.map((item) => {
                ...
                const sectionColor = SECTION_COLORS[group.section] || { bg: 'bg-primary/10', text: 'text-primary' }
                ...
                return (
                  ...
                  >
                    <Link
                      ...
                      onClick={() => onClose?.()}
                      className={cn(
                        "w-full flex items-center rounded-md text-xs font-semibold transition-all duration-150",
                        isSidebarCollapsed ? 'justify-center py-2.5 px-0' : 'gap-3 px-3 py-2',
                        isActive 
                          ? 'text-foreground bg-secondary/80 border-l-[3px] border-primary shadow-sm' 
                          : 'text-muted-foreground hover:text-foreground hover:bg-secondary/40'
                      )}
                    >
                      {/* Icon wrapped in a beautiful colored box */}
                      <div className={cn(
                        "flex items-center justify-center rounded-md shrink-0 transition-transform duration-200",
                        isSidebarCollapsed ? "w-8 h-8" : "w-6 h-6",
                        isActive ? "scale-105" : "hover:scale-105",
                        sectionColor.bg
                      )}>
                        <Icon
                          size={isSidebarCollapsed ? 18 : 13}
                          className={cn("shrink-0", sectionColor.text)}
                        />
                      </div>
                      {!isSidebarCollapsed && (
