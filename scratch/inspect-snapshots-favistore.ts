...
async function main() {
  ...
  const snapshots = await db.select().from(campaignsSnapshot).where(eq(campaignsSnapshot.customerId, "5541508335"));
  console.log(`Found ${snapshots.length} campaign snapshots for account 5541508335.`);
